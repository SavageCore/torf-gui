...
class CreateTorrentBatchQThread(QtCore.QThread):
    ...
    def __init__(self, path, exclude, save_dir, trackers, web_seeds,
                 private, source, randomize_infohash, comment, include_md5):
        super().__init__()
        self.path = path
...
class TorfGUI(Ui_MainWindow):
    ...
    def loadSettings(self):
        ...
        randomize_infohash = bool(int(settings.value('options/randomize_infohash') or 0))
    ...
    def saveSettings(self):
        ...
        settings.setValue('options/randomize_infohash', int(self.randomizeInfoHashCheckBox.isChecked()))
    ...
    def showAboutDialog(self):
        ...
        ad.dtVersionLabel.setText("(torf {})".format(
            torf.__version__))
        qdlg.exec_()
    ...
    def createButtonClicked(self):
        self.torrent.exclude_globs = self.excludeEdit.toPlainText().strip().splitlines()
        ...
        self.torrent.randomize_infohash = self.randomizeInfoHashCheckBox.isChecked()
    ...
    def createTorrentBatch(self):
        ...
        if save_dir:
            ...
            self.creation_thread = CreateTorrentBatchQThread(
                ...
                exclude_globs=self.excludeEdit.toPlainText().strip().splitlines(),
                save_dir=save_dir,
                trackers=trackers,
                webseeds=web_seeds,
